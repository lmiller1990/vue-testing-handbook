...
Forms are usually submitted to some endpoint. Let's see how we might test this component with a different implementation of `handleSubmit`. One common practise is to alias your HTTP library to `Vue.prototype.$http`. This allows to use make an ajax request by simply calling `this.$http.get(...)`. Learn more about this practice [here](https://vuejs.org/v2/cookbook/adding-instance-properties.html). 
...
First, inlcude the mock implementation of `this.$http` at the top, before the `describe` block:
...
Now the test passes. The source code for `flush-promises` is only about 10 lines long, if you are interested in Node.js it is worthing reading and understanding how it works.
