...
Так как роутер обычно включает в себя несколько компонентов, работающих вместе, тесты для роутера занимают место выше в [пирамиде тестирования](https://medium.freecodecamp.org/the-front-end-test-pyramid-rethink-your-testing-3b343c2bca51), прямо на уровне e2e/интеграционного уровня тестирования. Тем не менее, иметь несколько юнит тестов вокруг роутинга не помешает.

Как обсуждалось в предыдущих секциях, есть два способа тестирования компонентов, которые работают с роутером:

1. использовать настощий экземпляр роутера
...
Создадим простой `<App>`, у которого есть путь `/nested-child`. Перейдя на `/nested-child`, будет отрисован компонент `<NestedRoute>`. Сделаем файл `App.vue` и вставим следующий минималистичный компонент:
...
В настоящем приложение, вы обычно создаёте файл `router.js` и импортируете все созданные маршруты, а мы напишем так:
...
Так как мы не хотим засорять глобальное пространство имён, вызывая `Vue.use(...)` в наших тестах, создадим роутер внутри теста. Это даст нам больше контроля над состоянием приложения в течении юнит тестирования.
...
В нашем юнит тесте, вы __можете__ испортировать экземляр роутера и попытаться вызвать `beforeEach`, написав `router.beforeHooks[0]()`. Это приведёт к ошибки вокруг `next` – так как вы не передали правильные аргументы. Вместо этого применяется стратегия, при которой нужно отделить и независимо экспортировать хук навигации `beforeEach` перед его подключением к роутеру. Как насчёт такого:
...
Мы может тестировать это также, как и глобальные хуки:
...
- использование моков для Vue компонентов, с полмощью `jest.mock` и `localVue`
